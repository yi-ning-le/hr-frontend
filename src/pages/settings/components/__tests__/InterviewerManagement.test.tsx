      ...
      if (queryKey[0] === "employees") {
        return {
          data: {
            employees: [
              {
                id: "emp1", // Existing interviewer
                firstName: "John",
                lastName: "Doe",
                department: "Engineering",
              },
              {
                id: "emp2", // Candidate for interviewer
                firstName: "Jane",
                lastName: "Smith",
                department: "Design",
              },
...
describe("InterviewerManagement", () => {
  it("renders existing interviewers and allows interaction when admin", async () => {
    mockUseUserRole.mockReturnValue({ isAdmin: true, isLoading: false });
    const user = userEvent.setup();
    render(<InterviewerManagement />);

    // Verify existing interviewer is shown
    expect(screen.getByText("John Doe")).toBeInTheDocument();
    expect(screen.getByText("Engineering")).toBeInTheDocument();
    expect(screen.getByText("Interviewer Management")).toBeInTheDocument();

    // Verify "Add Interviewer" button
    const addButton = screen.getByText("Add Interviewer");
  ...
  it("shows access denied message when not admin", () => {
    ...
    expect(
      screen.getByText(/Admin access required to manage interviewers/i),
    ).toBeInTheDocument();
    expect(screen.queryByText("Add Interviewer")).not.toBeInTheDocument();
  });
});
